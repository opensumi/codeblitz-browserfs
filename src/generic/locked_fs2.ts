...
/**
 * like locked_fs，but this is use rwmutex.
 * for the scene that read more and write less, performance can be improved
 */
export default class LockedFS2<T extends FileSystem> implements FileSystem {
  private _fs: T;
  private _mu: RWMutex;

  constructor(fs: T) {
    this._fs = fs;
    this._mu = new RWMutex();
  }

  public getName(): string {
    return 'LockedFS2<' + this._fs.getName()  + '>';
  }
  ...
  public readFile(fname: string, encoding: string, flag: FileFlag, cb: BFSCallback<string | Buffer>): void {
    if (flag.isWriteable() && flag.getFlagString() !== 'r+') {
      this._mu.lock(() => {
